...
const BranchActions = (props) => {
  ...
  const { actions, reason, isVisible, isCategorySelected, category } = props
  return (
    <Container isVisible={actions && !!actions.length && isVisible}>
      <BranchActionsPopup actions={actions} reason={reason} category={category} isCategorySelected={isCategorySelected}>
        <Wrapper>
          <Round />
          <Round />
          <Round />
